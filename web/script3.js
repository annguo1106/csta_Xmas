...
const JSON_URL = 'https://docs.google.com/spreadsheets/d/1cY9AdxHpsh_sjY-BjXlf-sEfQFvW3-6zD9nzzRCXaeo/gviz/tq?tqx=out:json'; // 替換為你的表格 ID
...
async function fetchData() {
    try {
        ...
        const votes = { '交大-兩種角度': 0, '清大-礬': 0 }; // 預設清大、交大票數為 0

        // 計算每個選項的票數
        rows.forEach(row => {
            const choice = row.c[1]?.v; // 假設選項在第二列
            if (choice === '交大-兩種角度' || choice === '清大-礬') {
